...
test.readWrite.integer <- function() {
  ...
  checkIdentical(colnames(rdata), c("X1", "X2", "X3", paste("V", 4:12, sep = "")))
...
### test: read write empty vectors and matrices

test.readWrite.emptyVector <- function() {

    checkNull <- function( dat ) {
    ...
    x <- double()
      ...
      # write with colnames FALSE
    write.xls(x, wfile, colNames = FALSE)

    wdata <- read.xls(wfile, colNames = FALSE, type = "double")
    checkNull(wdata)
    wdata <- read.xls(wfile, colNames = TRUE, type = "double")
    checkNull(wdata)
    wdata <- read.xls(wfile, colNames = "custom", type = "double")
    checkNull(wdata)

      # write with colnames TRUE
    ...
    checkNull(wdata)
      ...
      # write with colnames given
    checkException(write.xls(x, wfile, colNames = "myColName"), silent = TRUE)
}
...
test.readWrite.emptyMatrix <- function() {
    ...
    checkIdentical(colnames(wdata), "V1")
    checkIdentical(rownames(wdata), "1")
    wdata <- read.xls(wfile, colNames = TRUE, type = "double")
    checkTrue(nrow(wdata) == 0)
    checkTrue(ncol(wdata) == 1)
    checkIdentical(colnames(wdata), "custCol")
    checkIdentical(rownames(wdata), NULL)
    wdata <- read.xls(wfile, colNames = "custom", type = "character")
    checkIdentical(wdata[[1]], "custCol")
    checkIdentical(colnames(wdata), "custom")
    checkIdentical(rownames(wdata), "1")
    ...
    checkTrue(nrow(wdata) == 0)
    checkTrue(ncol(wdata) == 1)
    checkIdentical(colnames(wdata), "myColName")
    checkIdentical(rownames(wdata), NULL)
    wdata <- read.xls(wfile, colNames = "custom", type = "character")
    checkIdentical(wdata[[1]], "myColName")
    checkIdentical(colnames(wdata), "custom")
    ...
    checkException(write.xls(x, wfile, colNames = c("myColName1", "myColName2")), silent = TRUE)
}


### test: read write data.frame
...
test.readWrite.dataFrame.1 <- function() {
  myval <- data.frame(Fertility = c(80.2, 83.1, 92.5, 85.8, 76.9, 76.1, 83.8, 92.4, 82.4, 82.9, 87.1, 64.1), 
    ...
    Testlogical = as.integer(c(NA, NA, NA, 1, 1, 0, rep(NA, 6))),
  ...
  mycls <- c("numeric", "numeric", "integer", "integer", "numeric", "numeric", "factor")
...
### test: read write empty data.frame

test.readWrite.emptyFrames <- function() {
    ...
    checkNull(wdata)
    wdata <- read.xls(wfile, colNames = "custom")
    checkNull(wdata)
    ...
    checkException(write.xls(x, wfile, colNames = "custCol"), silent = TRUE)
    ...
    wdata <- read.xls(wfile, colNames = FALSE)
    checkIdentical(as.character(wdata[[1]]), "custCol")
    checkIdentical(colnames(wdata), "V1")
    checkIdentical(rownames(wdata), "1")
    wdata <- read.xls(wfile, colNames = TRUE)
    checkIdentical(wdata[[1]], logical())
    checkTrue(nrow(wdata) == 0)
    checkTrue(ncol(wdata) == 1)
    checkIdentical(colnames(wdata), "custCol")
    checkIdentical(rownames(wdata), character())
    wdata <- read.xls(wfile, colNames = "custom")
    checkIdentical(as.character(wdata[[1]]), "custCol")
    checkIdentical(colnames(wdata), "custom")
    checkIdentical(rownames(wdata), "1")
